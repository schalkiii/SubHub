...
| **P15** | **修复已配置引擎时的假绿误报**：移除「本批全部探测失败则退回 TCP」的全局回退，改为只要配置引擎即逐节点以 gstatic 实测为权威，根除 `35.187.156.27:443` 类「端口可连、代理不通」误报 | ✅ |
...
```
- 应用启动后常驻 **系统托盘**：左键单击托盘图标恢复窗口，右键菜单含「显示 SubHub / 退出」。
- **点右上角关闭按钮 = 最小化到托盘**（不退出）：后台测速引擎继续运行；仅托盘菜单「退出」才真正结束进程。若需直接退出，从托盘菜单选择「退出」即可。
- 任务栏 / 托盘图标来自 `app/icons/`（`icon.ico` 等），由 `tauri icon` 从源图统一生成，请勿手改空文件。
...
| GET  | `/api/speedtest`     | 手动测速，返回 **SSE 流**（`text/event-stream`），WebUI 据此实时显示进度条与「正在测的节点 / Ping / 带宽」。查询参数：`timeout_ms`（默认 4000）、`concurrency`（默认 20）、`mode`（`all` 全部 / `untested` 只测 `last_tested_at==None` / `failed` 只测 `available==Some(false)`，复用自动刷新增量思路）、`ids`（可选，URL 编码的 JSON 数组字符串，形如 `ids=["<fp1>","<fp2>"]`，**只测这些选中节点**，供节点页「测速选中」多选使用；用 JSON 数组而非分隔符拼接，避免 fingerprint 内逗号（常见于 vless+ws 的 path）被误切导致匹配不到）。事件：`progress`（每个节点完成 **任一阶段** 发一条，含 `done/total/name/available/latency_ms/bandwidth_bps/phase`，`phase` 为 `tcp`/`http`/`bw`）与 `done`（汇总 `tested/reachable/avg_latency_ms`）。**进度总量覆盖全程**：无引擎时 `total = 节点数`（仅 TCP）；配置引擎时 `total = 节点数 × 3`（TCP + HTTP 延迟 + 带宽各占一段），所以进度条**真正到 100% 才结束**，不会在 TCP 完成后卡住。每个节点的 TCP 延迟 / 可用性（及可选 HTTP 延迟、下行带宽）测完即写回节点记录。**可用性判定对齐 clash-verge**：配置了 `SUBHUB_ENGINE_BIN` 且本批确有节点经引擎 gstatic generate_204 实测成功时，节点的「可用」就以**引擎的真实代理级 HTTP 探测**为准（逐节点 `h.is_some()`）——TCP 能连但协议层不通的节点会被正确判为不可用，不再出现「SubHub 绿、clash-verge 红」的假绿（例如 `35.187.156.27:443` 这类 Google 443 端口，TCP 必可连但代理隧道不通的节点）；当引擎未配置、或本批 0 个节点探测成功（引擎整体故障/超时过严）时，回退到裸 TCP 可用性作为兜底，避免 mass-red 让用户完全无法使用 |
...
| GET  | `/api/proxy-yaml`   | **取单个节点原始 YAML**（调试用）：`?fp=<fingerprint>` 返回该节点经 `to_clash_meta` 序列化的 clash-meta 配置（与测速引擎、导出**同源**）。便于把这份精确配置粘进 clash-verge 等客户端 1:1 比对，定位「SubHub 能通、某客户端不通」是否源于该客户端的订阅预处理/缓存 |
